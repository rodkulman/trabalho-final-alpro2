...
public class CustomStackedBarChart extends JFreeChart
{
	...
	{
		if (dataset.getRowIndex(row) < 0)
		{
			throw new InvalidParameterException("row doesn't belong to this chart.");
		}
		
		if (dataset.getColumnIndex(column) < 0)
		{
			throw new InvalidParameterException("column doesn't belong to this chart.");
		}
		
		renderer.setItemPaint(dataset.getRowIndex(row), dataset.getColumnIndex(column), color);
		
		// plot.getRenderer().setSeriesPaint(dataset.getRowIndex(row), color);
	}
