...
public class Cashiers implements Iterable<Cashier>
{
	...
	/**
	 * Iterates through the list looking for cashier with the specified
	 * parameter
	 * 
	 * @param query
	 *            Allows for custom filtering
	 * @return An Iterable that iterates though the list looking for cashier
			...
			public Iterator<Cashier> iterator()
			{
				return new Iterator<Cashier>()
				{
					...
					public boolean hasNext()
					{
						...
						{
							if (query.where(cashiers.get(index))) { return true; }
						}
